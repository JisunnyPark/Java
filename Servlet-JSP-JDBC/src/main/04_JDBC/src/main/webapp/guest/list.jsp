...
<%-- JDBC 프로그램으로 DB 데이터를 가져와서 화면 출력 --%>

<%
	// JDBC 프로그램을 위한 변수 선언 --> 변경될 일이 없으니 final로 선언!!
...
<body>
	...
	<table border>
		<thead>
			<tr>
				<th> 번호 </th> <%-- DB 테이블에 없는 값!! rownum 사용할 것임(자동 번호 생성 / ORDERING은 불가능) --%>
				<th> 사번 </th>
				<th> 성명 </th>
				<th> 날짜 </th>
				<th> 금액 </th>
...
<%
	...
	try { 
		...
		while (rs.next()) { %>
			...
			<tr>
				<td> <%=rs.getInt("RN") %> </td>
				<td> <%=rs.getString("SABUN") %> </td>
				<td> <%=rs.getString("NAME") %> </td>
				<td> <%=rs.getDate("NALJA") %> </td>
				<td> <%=rs.getInt("PAY") %> </td>
				<td> 
					<a href="detail.jsp?idx=<%=rs.getString("SABUN") %>"> 상세 보기 </a> 
				</td>
			</tr>
<% 
			} // end while
...
%>
	...
	<p> 전체 인원 : <%=cnt %> </p>
	<p><a href="addForm.jsp"> 사원 등록 </a></p>
</body>
</html>
