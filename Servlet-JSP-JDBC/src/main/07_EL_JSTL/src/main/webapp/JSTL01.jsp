...
--%>
	...
	<%-- set : 속성 선언 태그 setAttribute 역할
		<c:set var="속셩명" value="속성값" scope="범위" />
...
<% 
	// System.out.println("test : " + test); -- 사용 불가
...
%>
	...
	<p> application test(EL) : ${applicatonScope.test } </p>
	...
	<p> EL (application) : ${applicationScope.test } </p>
	<hr>
	...
	<p> (실습)1 ~ 10 까지의 숫자 중 짝수 출력(step="1")사용 </p>
	<c:forEach var="i" begin="1" end="10" step="1">
		<c:if test="${i % 2 == 0}">	
			${i }&nbsp;&nbsp;
		</c:if>
	...
	<p> (실습)1 ~ 10 까지의 숫자 중 짝수 출력(step="2")사용 </p>
	...
	<!--  ===== forEach문 집합 객체 처리 ===== -->
	<h2> forEach문 집합 객체 처리 </h2>
	<h3> 스크립트릿으로 배열 값 출력 </h3>
<%
...
%>
	...
	<h3> EL에서 스크립트릿 변수 사용(scope에 등록해야 사용 가능!!) </h3>
	<%--${arr } 실행 안됨--%>
	<c:set var="attr_names" value="<%=arr %>"/>
	
	<h3>==== forEach문으로 배열 데이터 처리 ====</h3>
	<c:forEach var="name" items="${attr_names }">
		${name },
	</c:forEach>
	<hr><hr>
	
	<%-- ===== forTokens 태그(문자열 다룰 때 사용) ===== --%>
	...
	<h3> forTokens 구분자 / 와 , 사용(두개 다 사용됨) </h3>
	<c:forTokens var="str" items="${names }" delims="/,">
		<p> ${str } </p>
	</c:forTokens>
</body>
